...
// First we have to iterate over the List to check if we already have a free
// space To avoid allocating when we can reuse.
...
struct BlockMeta *request_space(struct BlockMeta *last, size_t size) {
  ...
  // We request memory to fit users request + the metadata that's pointing to
  // the heap
...
void exercise1_malloc() {
  printf("Running exercise test, asserting if all data types can be used with "
         "my allocator\n");
}
